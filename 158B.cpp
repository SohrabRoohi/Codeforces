...
int main() {
    ...
    while (groups[1] > 1 && groups[2] > 0) {
        groups[1] -= 2;
        groups[2]--;
        cars++;
    }
    if(groups[1] == 1 && groups[2] > 0) {
        groups[1]--;
        groups[2]--;
        cars++;
    }
    double groupOfOne = (double)groups[1];
    double groupOfTwo = (double)groups[2];
    cars += ceil(groupOfOne / 4);
    cars += ceil(groupOfTwo / 2);
    cars += groups[3];
    cars += groups[4];
    cout << cars;
